...
- 숙소 설정은 복수 항목을 한 번에 저장한다. TourAPI 숙소를 고르면 기존 장소 좌표를 사용하고,
  직접 입력한 숙소명·주소는 서버가 카카오 로컬 API로 좌표를 조회한다. 조회에 실패해도 숙소는 저장되며
  좌표가 없으면 거리 기반 추천의 출발점으로만 사용하지 않는다.
- 여행 스타일 설정은 여행 강도(`RELAXED`/`TIGHT`), 복수 동행, 여행 MBTI를 저장한다.
...
- AI는 한국 시간 기준 도착·출발 범위 안에서 기존 일정 및 생성 항목끼리 겹치지 않게 배치한다.
  생성 중 추가한 수동 일정도 저장 직전에 다시 확인하며, 충돌하면 생성 실패로 처리한다.
- 비공연일 AI는 같은 여행 계획에서 등록한 장소를 제외한 좌표 보유 장소를 숙소 거리·여행 MBTI·동행 태그로 점수화한
  상위 10개 후보만 사용한다. 숙소 거리는 20km까지 점수에 반영하되 먼 장소도 후보에서 제외하지
  않으며, Gemini는 후보 재선별 단계 없이 일정 생성을 한 번만 호출한다.
...
동시 읽음 요청과 메시지 전송에서도 저장된 읽음 커서는 되돌아가지 않는다. 읽음 이벤트가
도착하는 순서는 달라질 수 있으므로, 프론트도 참여자별로 더 큰 `lastReadMessageId`만 반영한다.
