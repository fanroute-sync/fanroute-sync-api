...
public class GeminiRestClient {
  ...
  private String extractText(GeminiDto.GenerateContentResponse response) {
    ...
    if (!"STOP".equals(response.candidates().getFirst().finishReason())) {
      throw new IllegalArgumentException("Gemini response did not finish normally: "
  ...
  private String createItineraryPrompt(AiItineraryGenerationDto.GenerationInput input) {
    return "%s\n%s\n기존 일정과 겹치지 않는 일반 일정만 생성해라. 기존 일정은 결과에 포함하지 마라.\n"
        ...
        + "일반 일정은 여행 강도에 맞춰 최대 %d개만 생성해라. time은 HH:mm, durationMinutes는 1~720의 정수로 반환해라."
            .formatted(input.maxItems());
  }
  ...
  private static Map<String, Object> createItinerarySchema(AiItineraryGenerationDto.GenerationInput input) {
    List<Object> allowedPlaceIds = new ArrayList<>(input.placeCandidates().stream()
        .map(AiItineraryGenerationDto.PlaceCandidate::id).toList());
    allowedPlaceIds.add(null);
    Map<String, Object> itemSchema = Map.of(
        "type", "object",
        "additionalProperties", false,
        "properties", Map.of(
            "time", Map.of("type", "string", "description", "Asia/Seoul 기준 시작 시각, HH:mm"),
            "title", Map.of("type", "string"),
            "durationMinutes", Map.of("type", "integer", "minimum", 1, "maximum", 720),
            "placeId", Map.of("type", List.of("integer", "null"), "enum", allowedPlaceIds)),
        "required", List.of("time", "title", "durationMinutes", "placeId"));
    return Map.of(
        "type", "object",
        "additionalProperties", false,
        "properties", Map.of("items", Map.of("type", "array", "items", itemSchema,
            "minItems", 1, "maxItems", input.maxItems())),
