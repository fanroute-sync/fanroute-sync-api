...
class ApiResponseTest {
  ...
  void fail() {
    ApiResponse<Void> response = ApiResponse.fail(ErrorCode.DATA_NOT_FOUND);

    assertThat(response.success()).isFalse();
    assertThat(response.status()).isEqualTo(404);
    assertThat(response.code()).isEqualTo("DATA_NOT_FOUND");
    assertThat(response.message())
  ...
  void toResponseEntity() {
    ApiResponse<Void> response = ApiResponse.fail(ErrorCode.INVALID_PARAMETER);
